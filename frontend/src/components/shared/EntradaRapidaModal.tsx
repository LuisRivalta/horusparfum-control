...
const MOTIVOS = [
  { value: 'reposicao_manual', label: 'Reposição manual' },
  { value: 'ajuste_inventario', label: 'Ajuste de inventário' },
  { value: 'devolucao', label: 'Devolução' },
  { value: 'outro', label: 'Outro' },
]

export function EntradaRapidaModal({ open, onClose, onDone, produtoId }: Props) {
  const { user } = useAuth()
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.motivo) { setErro('Selecione o motivo'); return }
